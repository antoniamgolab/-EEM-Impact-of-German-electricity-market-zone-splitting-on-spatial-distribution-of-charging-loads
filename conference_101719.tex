...
\begin{abstract}
The potential splitting of Germany's unified electricity bidding zone into a wind-rich North and a demand-heavy South would create persistent regional price differentials. For long-haul battery-electric trucks that traverse multiple pricing regions, such differentials alter where along a route it is cheapest to charge, thereby reshaping the spatial distribution of charging infrastructure demand. We quantify this effect using a cost-minimizing linear program for multi-year fleet turnover and spatially resolved charging allocation on approximately 1,870 origin-destination freight flows along the Scandinavian-Mediterranean TEN-T corridor. Six zone-split scenarios---combining three North-South price spreads (5\%, 10\%, 20\%) with two implementation timings (from 2032, from 2040)---are compared against a unified-zone baseline. Results show that while the aggregate BEV fleet share is insensitive to the split, the spatial allocation of charging demand shifts substantially northward: in the most pronounced scenario (20\% spread from 2032), DE-North gains +111 MW of required charging capacity at 2040, while DE-South loses -87 MW. The redistribution propagates beyond German borders, with Austria losing up to 31 MW of required capacity. Total corridor-wide charging capacity remains constant across scenarios, confirming that the zone split redistributes---but does not reduce---aggregate demand. These findings highlight that electricity market zone design creates infrastructure planning externalities for the transport sector that extend across national borders.
\end{abstract}
...
Fig.~\ref{fig:elec_prices} shows the resulting electricity charging prices (including CPO markup) for the High~(20\%) spread scenario (split from~2032). The North--South differential is clearly visible: DE-North prices fall below the corridor-country range while DE-South prices rise above it. Non-German countries remain unaffected by the split.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{eem_electricity_prices_by_country_4scenarios.pdf}}
\caption{Fast-charging electricity prices for the High~(20\%) zone-split scenario (from~2032). Gray areas show diesel cost (base and including CO$_2$ price). The blue band shows the range of other corridor countries (NO, FI, SE, DK, AT, IT). DE-North (squares) and DE-South (triangles) diverge from~2032 onward.}
...
Fig.~\ref{fig:charging_delta} shows the change in annual charging demand relative to the baseline for each zone-split scenario. The redistribution is approximately symmetric: charging demand gained by DE-North mirrors the loss in DE-South. The effect scales with the price spread---the High~(20\%) from-2032 scenario shifts nearly 500~GWh/a northward by~2048, while even the Low~(5\%) scenario produces a shift of $\sim$270~GWh/a. Later implementation (from~2040 vs.\ from~2032) reduces the shift but does not eliminate it, as the cumulative fleet and infrastructure decisions made before the split constrain the system's response.
...
Because charging infrastructure is irreversible once deployed, the zone split creates a mismatch between where capacity was built (under baseline assumptions) and where it is actually needed. Fig.~\ref{fig:used_unused} compares installed capacity utilization for the baseline and the High~(20\%) from-2032 scenario. In DE-South, the split causes substantial overcapacity (hatched bars), as demand migrates northward but previously installed chargers remain. In DE-North, the opposite occurs: required capacity exceeds the baseline build-out, creating a deficit that must be met by accelerated infrastructure investment.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{eem_used_vs_unused_capacity.pdf}}
\caption{Installed charging capacity decomposed into used (solid) and unused (hatched) components for DE-North (top) and DE-South (bottom). Blue: baseline (unified zone); red: High~(20\%) zone split from~2032.}
\label{fig:used_unused}
\end{figure}

The redistribution of charging demand within Germany propagates to neighbouring countries along the corridor. Table~\ref{tab:mw_delta} reports the change in required charging capacity (MW) relative to the baseline for each zone and scenario at~2040. DE-North gains up to $+111$~MW of required capacity (H-2032), while DE-South loses $-87$~MW. Notably, Austria loses up to $-31$~MW in the most extreme scenario (H-2032) as the large price spread pulls charging demand back into cheaper DE-North, but shows small increases ($<+4$~MW) in all other scenarios. Italy gains up to $+12$~MW in the H-2032 case. Denmark is essentially unaffected ($<1$~MW). The total corridor capacity remains constant across scenarios ($\sim$559~MW), confirming that the zone split redistributes---but does not reduce---aggregate charging demand. These results show that a unilateral zone-split decision in Germany creates infrastructure planning externalities for transit countries.
